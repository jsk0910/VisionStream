import os
import sys
import glob
import time
import torch
import torchvision.io as io
from pytorch_msssim import ms_ssim

sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../')))
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '../build')))
from user_workspace.custom_codecs.learned_compression.model_v2 import HybridCompressionModelV2
import visionstream as vs  # compiled C++ core

def pad_image(x, p=64):
    h, w = x.shape[2], x.shape[3]
    h_pad = (p - h % p) % p
    w_pad = (p - w % p) % p
    padding = (0, w_pad, 0, h_pad)
    return torch.nn.functional.pad(x, padding, mode='constant', value=0.0), h_pad, w_pad

def get_dummy_cdf(max_cdf_length=256, precision=16):
    total = 1 << precision
    cdf = [0]
    for i in range(max_cdf_length - 1):
        pm = max(1, int(total * (0.5 ** (i+1))))
        cdf.append(min(total, cdf[-1] + pm))
    cdf[-1] = total
    # Ensure strict monotonicity
    for i in range(len(cdf)-1):
        if cdf[i+1] <= cdf[i]:
            cdf[i+1] = cdf[i] + 1
    if cdf[-1] > total:
        cdf = [int((val / cdf[-1]) * total) for val in cdf]
        cdf[-1] = total
    return cdf

def evaluate():
    print("=== VisionStream Phase 6: Next-Gen Hybrid Codec Evaluation ===")
    
    device = "cuda" if torch.cuda.is_available() else "cpu"
    
    # 1. Load V2 Model
    model = HybridCompressionModelV2(device=device).to(device)
    model.eval()
    
    ckpt_path = os.path.abspath(os.path.join(os.path.dirname(__file__), '../user_workspace/custom_codecs/learned_compression/checkpoint_v2.pth'))
    if os.path.exists(ckpt_path):
        print(f"Loading V2 checkpoint from: {ckpt_path}")
        model.load_state_dict(torch.load(ckpt_path, map_location=device))
    else:
        print(f"[Warning] No V2 checkpoint found at {ckpt_path}.")
        
    # 2. Dataset Setup
    data_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '../data/kodak'))
    image_paths = sorted(glob.glob(os.path.join(data_dir, "*.png")))
    if not image_paths:
        print("No Kodak images found. Exiting.")
        return
        
    print(f"Evaluating V2 architecture on {len(image_paths)} Kodak images...\n")
    
    dummy_cdf = get_dummy_cdf()
    cdf_sizes = [256]
    offsets = [0]
    
    total_bpp = 0.0
    total_psnr = 0.0
    total_msssim = 0.0
    total_ae_time = 0.0
    total_ad_time = 0.0
    total_nn_time = 0.0

    print(f"{'Image':<12} | {'PSNR (dB)':<10} | {'MS-SSIM':<8} | {'BPP':<8} | {'AE ms':<7} | {'AD ms':<7} | {'NN ms':<7}")
    print("-" * 75)

    for idx, path in enumerate(image_paths):
        filename = os.path.basename(path)
        img_tensor = io.read_image(path).float() / 255.0
        img_tensor = img_tensor.unsqueeze(0).to(device)
        
        img_padded, h_pad, w_pad = pad_image(img_tensor, p=64)
        orig_h, orig_w = img_tensor.shape[2], img_tensor.shape[3]
        
        torch.cuda.synchronize()
        t0 = time.time()
        
        with torch.no_grad():
            # Encoder side (Encoder V2 + HyperEncoder V2)
            y = model.encoder(img_padded * 255.0)
            y_hat = model.quantize(y, is_training=False)
            
            z = model.hyper_encoder(y)
            z_hat = model.quantize(z, is_training=False)
            
            # --- Entropy C++ AE Simulation ---
            y_sym = torch.clamp(y_hat + 128, 0, 255).to(torch.uint8).flatten().cpu().tolist()
            z_sym = torch.clamp(z_hat + 128, 0, 255).to(torch.uint8).flatten().cpu().tolist()
            
            all_syms = y_sym + z_sym
            all_indexes = [0] * len(all_syms)

            t1 = time.time()
            bitstream = vs.ArithmeticCoder.encode(all_syms, all_indexes, dummy_cdf, cdf_sizes, offsets, 16)
            t2 = time.time()
            
            _ = vs.ArithmeticCoder.decode(bitstream, all_indexes, dummy_cdf, cdf_sizes, offsets, 16)
            t3 = time.time()
            
            # Decoder side
            psi = model.hyper_decoder(z_hat)
            # params = model.entropy(y_hat, psi) # GMM Params evaluation
            
            x_hat_padded = model.decoder(y_hat) / 255.0
            x_hat = x_hat_padded[:, :, :orig_h, :orig_w]
            
            torch.cuda.synchronize()
            t4 = time.time()
            
            mse = torch.nn.functional.mse_loss(x_hat, img_tensor)
            psnr = 10 * torch.log10(1.0 / mse) if mse > 0 else 100.0
            msssim_val = ms_ssim(x_hat, img_tensor, data_range=1.0, size_average=True)
            
            bpp = (len(bitstream) * 8) / (orig_h * orig_w)
            
            total_bpp += bpp
            total_psnr += psnr.item()
            total_msssim += msssim_val.item()
            total_nn_time += (t1 - t0) + (t4 - t3)
            total_ae_time += (t2 - t1)
            total_ad_time += (t3 - t2)
            
            print(f"{filename:<12} | {psnr.item():.2f}       | {msssim_val.item():.4f}   | {bpp:.4f}   | {(t2-t1)*1000:.1f}    | {(t3-t2)*1000:.1f}    | {((t1-t0)+(t4-t3))*1000:.1f}")
            
    # Baseline Phase 5 hardcoded references
    p5_psnr = 5.70
    p5_bpp = 12.7501
    p5_nn = 74.78
    
    v2_psnr = total_psnr / len(image_paths)
    v2_msssim = total_msssim / len(image_paths)
    v2_bpp = total_bpp / len(image_paths)
    v2_nn = (total_nn_time / len(image_paths))*1000
    v2_ae = (total_ae_time / len(image_paths))*1000
    v2_ad = (total_ad_time / len(image_paths))*1000

    print("\n" + "="*50)
    print("=== FINAL COMPARISON: Phase 5 vs Phase 6 (V2) ===")
    print("="*50)
    print(f"{'Metric':<25} | {'Phase 5':<10} | {'Phase 6 (V2)':<10} | Difference")
    print("-" * 65)
    print(f"{'Average PSNR (dB) Higher=':+<25} | {p5_psnr:<10.2f} | {v2_psnr:<10.2f} | {v2_psnr-p5_psnr:+.2f}")
    print(f"{'Average MS-SSIM Higher=':+<25} | {'N/A':<10} | {v2_msssim:<10.4f} | {'N/A':<10}")
    print(f"{'Average BPP (Lower=+)':<25} | {p5_bpp:<10.4f} | {v2_bpp:<10.4f} | {v2_bpp-p5_bpp:+.4f}")
    print(f"{'Average NN Latency (ms)':<25} | {p5_nn:<10.2f} | {v2_nn:<10.2f} | {v2_nn-p5_nn:+.2f}")
    print(f"{'Average C++ AE Encode ms':<25} | 96.19      | {v2_ae:<10.2f} | {v2_ae-96.19:+.2f}")
    print(f"{'Average C++ AE Decode ms':<25} | 67.36      | {v2_ad:<10.2f} | {v2_ad-67.36:+.2f}")
    print("="*50)

if __name__ == "__main__":
    evaluate()
