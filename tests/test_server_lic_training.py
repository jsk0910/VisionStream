...
from user_workspace.custom_codecs.learned_compression.entropy_gmm import RateDistortionLossV2
...
def run_tests():
    ...
    try:
        ...
        loss, out_metric = criterion(output, img)
        print(f"  - Loss Total: {loss.item():.4f} | BPP: {out_metric['bpp']:.4f} | MSE: {out_metric['mse']:.4f}")
        ...
        if has_grads:
            print(f"  - Gradients calculated successfully. (Encoder norm: {enc_grad.norm().item():.4f})")
            optimizer.step()
